...
# Usage:
# download MNIST and then make sure backprop_nn.py in the same directory.
...
def FeedForward(X_train_, Y_train_, X_test, Y_test, hidden, activations, activations_, eta_=0.1, batch_size=128, Steps = 150, holdoutRatio = 0.2,momentum=0):
    ...
    for e in range(Steps):
        ...
        while(i >= 0):
            ...
            if i == 0:    
                upd = prev_mom[i] - eta*np.dot(np.hstack((X_tra, np.ones([X_tra.shape[0],1]))).T, delta[i][:,:-1])/X_tra.shape[0]
            elif i == len(laWeights)-1:
                upd = prev_mom[i] - eta*np.dot(Z[i-1].T, delta[i])/X_tra.shape[0]
            else:
                upd = prev_mom[i] - eta*np.dot(Z[i-1].T, delta[i][:,:-1])/X_tra.shape[0] 
            Weights[i] = Weights[i] - upd
            prev_mom[i] = upd
            laWeights[i] = Weights[i] - upd
            i-=1
        ...
        summary.append(calculateLoss(X_train, Y_train, A, Z))
        if e%10 == 0:
            print('Step = ',e,'Train Acc = ',training_accuracy[e],'Valid Acc = ',validation_accuracy[e],'Train Loss = ',training_loss[e],'Valid Loss = ',validation_loss[e])
            summary_steps[e] = sum(summary)/len(summary)
            summary = []
...
def main():
    ...
    ep = 300
    idx,tr_a,va_a,tr_l,va_l,te_acc,final_weights = FeedForward(images_train,labels_train, images_test, labels_test, [300,200], [sigmoid, sigmoid, softmax],[sigmoid_,sigmoid_],eta_=0.1,momentum=0.0,batch_size=256, Steps=ep)
    print('Best Fit Obtained at Step : ',idx)
    ...
    print('Testing error rate at best fit : ',1-te_acc)
    # naming the x axis
    plt.xlabel('Step')
    # naming the y axis
    plt.ylabel('Loss')
    ...
    lists = sorted(summary_steps.items()) # sorted by key, return a list of tuples
