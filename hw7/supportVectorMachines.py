...
def formattedOutput(objFit):
    ...
    print("{:>1} out of {:>1} predictions correct.".format(objFit.number_of_test_examples,objFit.correct_predictions))
    print("accuracy:{:>1} ".format(objFit.correct_predictions/objFit.number_of_test_examples))
    print(dash)
...
def run_custom_svm(kernel='linear', C=0, gamma=0.1, degree=3):
    ...
    # X_train, y_train, X_test, y_test, X_val, y_val = datageneration.Data.split_data(X1, y1, X2, y2,percent)   # Break up examples into training and test data.
    X_train, y_train, X_test, y_test, X_val, y_val = objData.generate_dataset2()
...
if __name__ == "__main__":
    # run_custom_svm(kernel='linear',C=0, gamma=0.0)
    # run_custom_svm(kernel='linear',C=100)
    # run_custom_svm('polynomial',C=0, gamma=0.0, degree=3)
    # run_custom_svm('rbf', C=0, gamma=0.0)
